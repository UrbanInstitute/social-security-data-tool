...
(function (Highcharts) {
    ...
    function getContent(chart, href, extension, content, MIME) {
        ...
        if (chart.options.exporting.filename) {
            ...
            var tmp = chart.title.textStr.split("chartTitle")[1]
            tmp = tmp.replace(/\"/g,"").replace("</div>","").replace(/\>/g,"").replace(/\./,"").replace(/\\/g,"").replace(/\//g,"").replace(/\,/g,"")
            name = tmp.replace(/ /g, '_').toLowerCase();
        } else {
            name = 'chart';
        }
